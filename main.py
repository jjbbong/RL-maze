...
if __name__ == '__main__':
    ...
    end = get_end(maze_path)
    lr = 0.53
    discount = 0.95
    epsilon = 0.2
    actions = [(-1, 0), (1, 0), (0, -1), (0, 1)]  # 上,下,左,右

    if maze[1][end[0]][end[1]] == 1:
        print('你选择的是墙壁，无法到达')
    else:
        agent = QLearningAgent(alpha=lr, gamma=discount, epsilon=epsilon, actions=range(len(actions)), start=start)
        env = MazeEnvironment(maze[0], end, start, maze_path, block_size)
        for i in range(1000):
            state = start
            while True:
